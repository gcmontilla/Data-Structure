...
public:
    ...
    public:
        ...
        friend bool operator ==(const Iterator& lhs, const Iterator& rhs){
            return lhs.it == rhs.it and lhs.key_ptr == rhs.key_ptr;
        }

        friend bool operator !=(const Iterator& lhs, const Iterator& rhs){
            return lhs.it != rhs.it or  lhs.key_ptr != rhs.key_ptr;
        }
        void print_Iterator(){
            cout << *it << endl;
        }
    ...
    void copy_tree(const BPlusTree<T>& other, vector<BPlusTree<T>*> v);
...
template<typename T>
void BPlusTree<T>::copy_tree(const BPlusTree<T> &other, vector<BPlusTree<T> *> v) {
...
typename BPlusTree<T>::Iterator BPlusTree<T>::begin(){
    ...
    while(!ptr->is_leaf()){ ptr=ptr->subset[0]; }
...
typename BPlusTree<T>::Iterator BPlusTree<T>::upper_bound(const T &key){
    ...
    while(ptr!=NULL){
        for(int i =0;i<data_count;i++){
            if(ptr->data[i]>=key){
                return ++Iterator(ptr,data_count-1);
            }
        }
        ptr = ptr->next;
    }
